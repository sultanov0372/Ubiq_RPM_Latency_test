        ...
        public void Grasp(bool grasp)
        {
            if (grasp)
            {
                if (contacted != null)
                {
                    ...
                    // Set isKinematic to true when the object is grasped
                    var networkedObject = contacted.GetComponentInParent<NetworkedGravityChange>();
                    if (networkedObject != null)
                    {
                        networkedObject.isKinematic = true;
                    }
            ...
            else
            {
                if (grasped != null)
                {
                    // Set isKinematic to false when the object is released
                    var networkedObject = contacted.GetComponentInParent<NetworkedGravityChange>();
                    if (networkedObject != null)
                    {
                        networkedObject.isKinematic = false;
                    }
